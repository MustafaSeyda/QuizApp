...
public class test4 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        soru = dao.soru_çekme_iki_secenek(vt, "test4");
    ...
    public void go_back(View view) {
        Intent intent = new Intent(test4.this, testler.class);
    ...
    public void next(View view) {
        if (nextbtn.getText().equals("Finish")) {
            ...
            if (counter.sonuç(sayaç, 10, 2) == 1) {
                result1.setText("KENDİ İŞİNİN PATRONU OLMAK İSTİYORSUN DEMEK. NETWORK ÜN BOL OLSUN.(FREELANCE)");
                sp4=this.getSharedPreferences("com.mseyda.cengometre", Context.MODE_PRIVATE);
                sp4.edit().putString("test4",getString(R.string.test4a)).apply();




            } else if (counter.sonuç(sayaç, 10, 2) == 2) {
                result1.setText("ŞİRKETLERE OLAN HEVESİN ÇOCUKLUKTAN MI GELİYOR YOKSA İMKANLARI MI SENİ CEZBEDİYOR?");
                sp4=this.getSharedPreferences("com.mseyda.cengometre", Context.MODE_PRIVATE);
                sp4.edit().putString("test4",getString(R.string.test4b)).apply();
